...
sub apply_tagspec($$$$) {
  ...
  FN0: for my $fn0 (@$filenames) {
    ...
    if (!defined $got or $got<0) {
      if ($!{EADDRNOTAVAIL} or "$!" eq "Cannot assign requested address") {  # This doesn't happen with locfileorg.
...
* Lines of the setfattr form: setfattr -n user.lfo.tags -v '<tags>' '<filename>'
* Lines of the setfattr form: setfattr -x user.lfo.tags '<filename>'
