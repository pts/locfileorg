...
my $known_tags = read_tags_file("$ENV{HOME}/.locfileorg_tags");
my($C, $KC, $EC) = (0, 0, 0);
...
sub apply_tagspec($$$$) {
  ...
  FN0: for my $fn0 (@$filenames) {
    ...
    if (!defined $got or $got<0) {
      if ($!{EADDRNOTAVAIL} or "$!" eq "Cannot assign requested address") {  # This does not happen with locfileorg.
...
* Lines of the setfattr form: setfattr -n user.lfo.tags -v \x27<tags>\x27 \x27<filename>\x27
* Lines of the setfattr form: setfattr -x user.lfo.tags \x27<filename>\x27
* Lines of the mediafileinfo form: format=... ... tags=<tags> ... f=<filename>
* Output of: getfattr -hR -e text -n user.lfo.tags --absolute-names <path>
...
if (@ARGV == 2 and $ARGV[0] eq "--stdin" and $ARGV[1] ne "-" and substr($ARGV[1], 0, 2) ne "--") {
  ...
  while (defined($line = <$f>)) {
    ...
    } elsif ($line =~ m@^setfattr[ \t]+-x[ \t]+user.lfo.tags[ \t]+(?:--[ \t]+)?($sharg_re)[ \t]*$@o) {
      my $filename = $sharg_decode->($1);
      apply_tagspec($tagspec_prefix, ($mode or "."), [$filename], 1);
    } elsif ($line =~ m@^setfattr[ \t]+-n[ \t]+user.lfo.tags[ \t]+-v[ \t]+($sharg_re)[ \t]+(?:--[ \t]+)?($sharg_re)[ \t]*$@o) {
...
my $dump_func =
    ($format eq "sh") ? sub {
      my($tags, $filename) = @_;
      length($tags) ?
          "setfattr -n user.lfo.tags -v " . fnq($tags) . " -- " . fnq($filename) . "\n" :
          "setfattr -x user.lfo.tags -- " . fnq($filename) . "\n"
    } : ($format eq "colon") ? sub {
      my($tags, $filename) = @_;
      "$tags :: $filename\n"
      ...
      # user.lfo.tags extended attribute). Use _mmfs_dump --print-empty=no
      # to get this behavior.
      "# file: $filename\nuser.lfo.tags=" . gfaq($tags). "\n\n"
    } : ($format eq "mfi") ? sub {
      my($tags, $filename) = @_;
      my $tagsc = $tags;
      $tagsc =~ s@[\s,]+@,@g;
