...
# eval "$(perl -x .../locfileorg.pl --load)"
# , and then call _mmfs etc. interactively.
#
# TODO(pts): Rename mmfs, movemetafs, locfileorg.
#
...
sub read_tags_file(;$) {
  my $tags_fn = $_[0];
  $tags_fn = "$ENV{HOME}/.locfileorg_tags" if !defined($tags_fn);
  my $F;
...
sub parse_dump($$) {
  ...
  while (defined($line = <STDIN>)) {
    ...
    } elsif ($line =~ m@^setfattr[ \t]+-x[ \t]+user.lfo.tags[ \t]+(?:--[ \t]+)?($sharg_re)[ \t]*\n@o) {
      my $filename = $sharg_decode->($1);
      $process_func->($filename, "", ".");
    } elsif ($line =~ m@^setfattr[ \t]+-n[ \t]+user.lfo.tags[ \t]+-v[ \t]+($sharg_re)[ \t]+(?:--[ \t]+)?($sharg_re)[ \t]*\n@o) {
...
{
  die1 "$0: fatal: not supported with locfileorg\n";
}
...
$topcmd = "_mmfs" if !length($topcmd) or $topcmd eq "locfileorg";  # Legacy.
